...
class myModel(nn.Module):
    ...
    def nll_loss(self, y_pred, y, y_mask, avg=True):
        #2.5公式(7)
        #print(y.shape) #(max_seq_length,batch_size)
        #print(y_pred.shape) #(max_seq_length，batch_size，vocab_size)
    ...
    def fc_nll_loss_mean(self, y_pred, y, y_mask, gamma=None, avg=True):
        '''
        add by lyy 
        在计算loss时除了希望当前vocab的值大,同时希望其他值小
        '''
        #print(y.shape) #(max_seq_length,batch_size)
        #print(y_pred.shape) #(max_seq_length，batch_size，vocab_size)
        if gamma is None:
    ...
    def forward(self, text_data, in_mask_matrix, in_tag_matrix, fine_tune=False, gamma=None):
        ...
        #按num_class维做softmax，每个数在(0,1)之间,并放大了差距，probs的shape与sequence_emissions相同
        #print(sequence_emissions[0])
        ...
        # bert finetune loss，从encoder的输出直接对应vocab的分类结果，直接计算loss，如文章2.5节所述，
        if self.loss_type == 'FC_FT_CRF_MEAN':
            loss_ft_fc, g = self.fc_nll_loss_mean(probs, tag_matrix, mask_matrix, gamma=gamma)
        elif "FC" in self.loss_type:
            #公式（10）
        ...
        #CRF层，在crf_layer.py中定义
        #输入 sequence_emissions 为encoder输出的分类 [seq_len, batch_size, num_class]
        #输入 tag_matrix 为目标正确句每个字对应index的序列
        #输出 CRF层forward后的loss，如文章2.5节所述
        ...
        elif self.loss_type == 'FC_FT_CRF_MEAN':#add by lyy
            loss = loss_ft_fc + loss_crf_fc
            return self.decode_result, loss, loss_crf_fc.item(), loss_ft_fc.item()
        else:
            print("error")
...
if __name__ == "__main__":
    ...
    #1.一个bert encoder，使用\model\bert\bert.ckpt中的预训练参数加载模型，模型定义在bert.py中
    ...
    for epoch in range(num_epochs):
        ...
        for train_step in range(train_step_num):
            ...
            # train_batch_text_list 大小为 (batch_size,此batch内的最长句子长度)，为list类型
            #第一维是batch中每个句子，第二维是该句子的每个中文字符，为待改正的错句
            
            # tag target matrix
            train_tag_matrix = process_batch_tag(train_batch_tag_list, nerdata.label_dict)
            # train_tag_matrix 大小为 (batch_size,此batch内的最长句子长度)，为numpy.ndarray类型
            #第一维是batch中每个句子，第二维是该句子的每个中文字符在vocab.txt中对应的index，为标准答案的正确句
            
            # tag mask matrix
            train_mask_matrix = make_mask(train_batch_tag_list)
            # train_mask_matrix 大小为 (batch_size,此batch内的最长句子长度)，为numpy.ndarray类型
            #第一维是batch中每个句子，第二维是0或1，1表示该字符有意义，0表示该字符无意义
            ...
            if acc_bs % args.print_every == 0:
                '''
                print(type(train_batch_text_list))
                print(len(train_batch_text_list))
                print(train_batch_text_list[0])
                print(type(train_tag_matrix))
                print(train_tag_matrix.shape)
                print(train_tag_matrix[0])
                print(type(train_mask_matrix))
                print(train_mask_matrix.shape)
                print(train_mask_matrix[0])
                '''
            ...
            #if acc_bs % (train_step_num) == 0:#每个epoch保存一次参数
                model.eval()
                gold_tag_list = []
                pred_tag_list = []
                with torch.no_grad():
                    ...
                    writer.add_scalar('loss', loss_accumulated / batches_processed, global_step=None, walltime=None)
                    writer.add_scalar('loss_crf', loss_crf_accumulated / batches_processed, global_step=None, walltime=None)
                    writer.add_scalar('loss_ft', loss_ft_accumulated / batches_processed, global_step=None, walltime=None)
