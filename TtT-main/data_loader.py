...
class DataLoader:
    def __init__(self, train_path, dev_path, test_path, label_dict, train_max_len):
        ...
        #print(type(self.train_text_list))
        #print(self.train_text_list[0])
        #print(type(self.train_tag_list))
        #print(self.train_tag_list[0])
        
        self.train_num = len(self.train_text_list)
        self.dev_num = len(self.dev_text_list)
        self.test_num = len(self.test_text_list)
        
        print ('from data_loader.py:')
    ...
    def get_next_batch(self, batch_size, mode):
        ...
        #print(batch_text_list[0])
        #print(batch_tag_list[0])
    ...
    def process_file(self, in_path):
        #调用process_one_line函数读入data并做变长处理
        all_text, all_tag = [], []
        with open(in_path, 'r', encoding = 'utf8') as i:
            lines = i.readlines()
            for l in lines:
                one_text, one_tag = self.process_one_line(l)
                #one_text, one_tag = self.process_one_line_split_by_word(l)
    ...
    def process_one_line(self, line):
        #实现的变长的处理，具见论文2.2节所述， text_list 为原始错句， tag_list 为正确句对应的index序列
    ...
    def process_one_line_split_by_word(self, line):
        '''
        add by lyy
        读入是首先对其进行中文分词（jieba），再以词为token继续进行变长处理
        '''
        #实现的变长的处理，具见论文2.2节所述， text_list 为原始错句， tag_list 为正确句对应的index序列
        content_list = line.strip().split('\t')
        assert len(content_list) == 3
        text_list = [w for w in jieba.cut(content_list[0].strip())] #+ ['<-SEP->']
        tag_name_list = [w for w in jieba.cut(content_list[1].strip())] + ['<-SEP->'] 
        if len(tag_name_list) > len(text_list):
            text_list += ['<-MASK->'] * (len(tag_name_list) - len(text_list))
            text_list += ['<-SEP->']
            tag_name_list += ['<-SEP->']
